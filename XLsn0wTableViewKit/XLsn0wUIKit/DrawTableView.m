...
- (void)drawCell:(XLsn0wTableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    XLsn0wTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (cell==nil) {
        cell = [[XLsn0wTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)loadContent{
    ...
    if (self.visibleCells&&self.visibleCells.count>0) {
        for (id temp in [self.visibleCells copy]) {
            XLsn0wTableViewCell *cell = (XLsn0wTableViewCell *)temp;
            [cell draw];
        }
    }
}
...
- (void)removeFromSuperview {
    for (UIView *temp in self.subviews) {
        for (XLsn0wTableViewCell *cell in temp.subviews) {
            if ([cell isKindOfClass:[XLsn0wTableViewCell class]]) {
                [cell releaseMemory];
            }
        }
    }
